...
class ModelBuilder:
    ...
    def fit(self, epoch, device=Utility.getDevice()):
        ...
        for e in range(0, epoch):
            ...
            print(f'Train Accuracy: {trainAcc}%, Train Loss: {train_result.loss}')
...
class ModelTrainer:
    ...
    def __train_one_batch(self, model, data, target, optimizer, lossFn):
        optimizer.zero_grad()
        output = model(data)
        loss = lossFn.computeLoss(model, output, target)
        loss.backward()
        optimizer.step()
...
class ModelBuildResult:
    ...
    def __init__(self, trainAccuracies, trainLosses, testAccuracies, testLosses):
